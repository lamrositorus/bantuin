                ...
                {requests.map((request) => (
                  <div key={request.id} className={`p-4 rounded-lg  shadow-md border border-gray-200 hover:shadow-xl transition-shadow duration-300 flex flex-col ${isDarkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-800'}`}>
                    ...
                    <div className="mt-4 text-center">
                      <Link to={`/getRequest/${request.id}`} className={`inline-block font-semibold py-2 px-4 rounded-lg transition duration-200 ${isDarkMode ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
                        View Details
                      </Link>
                    </div>
                  </div>
