...
if (data.password && data.password.length < 6) errors.password="Password minimal 6 karakter" ; if
  (!validatePhoneNumber(data.phone_number)) errors.phone_number="Format nomor telepon tidak valid" ; if (!data.address
  || data.address.length < 10) errors.address="Alamat minimal 10 karakter" ; if (!data.gender)
  errors.gender="Jenis kelamin harus dipilih" ; return errors; }; export const Profile=()=> {
  ...
  <div className="bg-gray-50 p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow">
    ...
    {isEditing ? (
    <div>
      <input type={type} value={localValue} onChange={handleInputChange} onBlur={handleBlur} className={`w-full bg-white
        p-3 rounded-xl border ${ error ? 'border-red-500' : 'border-gray-200' } focus:border-blue-500 focus:ring-2
    ...
    <div className="bg-white shadow-2xl rounded-2xl p-8 backdrop-blur-sm">
      ...
      <div className="mt-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {isEditing && (
          <EditableField label="Password" field="password" value={editedProfile.password || '' } type="password"
          ...
          <div className="bg-gray-50 p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow">
            ...
            <select value={editedProfile.gender || '' } onChange={(e)=> setEditedProfile((prev) => ({ ...prev, gender:
              e.target.value }))}
              className="w-full bg-white p-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2
              focus:ring-blue-200 transition-all duration-200"
              >
